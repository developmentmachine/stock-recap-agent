...
""",
    ...
    parser.add_argument("--provider", choices=["live", "akshare", "mock"], default="live")
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="模型表达：openai:<m> / ollama:<m> / cursor-agent",
    )
    ...
    parser.add_argument("--cursor-agent-cmd", type=str, default=None)
    ...
    if args.cursor_agent_cmd:
        settings.cursor_agent_cmd = args.cursor_agent_cmd
...
def _cmd_serve(settings: Any, logger: logging.Logger, args: argparse.Namespace) -> int:
    ...
    from stock_recap.api.routes import app

    scheduler = None
    if settings.scheduler_enabled:
        from stock_recap.scheduler.jobs import start_scheduler
...
def _cmd_push_test(settings: Any, logger: logging.Logger) -> int:
    if not settings.wxwork_webhook_url:
        ...
        return 1
    from stock_recap.push.wechat import test_push
...
def _cmd_history(settings: Any, logger: logging.Logger, args: argparse.Namespace) -> int:
    from stock_recap.db import load_history
...
def _cmd_generate(settings: Any, logger: logging.Logger, args: argparse.Namespace) -> int:
    ...
    from stock_recap.db import load_feedback_summary
    ...
    if args.dry_run:
        ...
        from stock_recap.llm.backends import llm_backend_effective, model_effective
        print(
            _stable_json(
                {
                    "llm_backend": llm_backend_effective(req.model),
