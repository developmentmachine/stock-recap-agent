...
- **内置**：``stock_recap.skills.manifest.json`` + 包内相对 ``path``。
...
- **Entry points**：第三方包在 ``pyproject.toml`` 登记::
      ...
      [project.entry-points."stock_recap.skills"]
...
_ENTRY_GROUP = "stock_recap.skills"
...
def _iter_entry_point_bundle_roots() -> List[Path]:
    ...
    for ep in selected:
        try:
            ...
            logger.warning(
                "failed to load stock_recap.skills entry point %s: %s", ep.name, e
            )
            continue
        try:
            if callable(obj):
