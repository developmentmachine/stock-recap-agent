...
PROMPT_VERSION 管理：
- 基础版本来自 prompts.PROMPT_BASE_VERSION
- 进化触发后，如 LLM 建议 bump，则自动递增 v1/v2/v3...
- 当前活跃版本存在内存（进程重启后从 DB 最新 evolution_notes 恢复）
"""
...
from stock_recap.llm.prompts import PROMPT_BASE_VERSION
from stock_recap.models import EvolutionNote, Features, MarketSnapshot, Mode

logger = logging.getLogger("stock_recap.memory")

# ─── 全局活跃 PROMPT_VERSION（内存缓存） ────────────────────────────────────────
_active_prompt_version: Optional[str] = None
...
def get_prompt_version(db_path: str) -> str:
    """获取当前活跃的 PROMPT_VERSION。优先内存缓存，其次从 DB 恢复。"""
    global _active_prompt_version
    if _active_prompt_version:
        return _active_prompt_version

    # 从 DB 恢复：取最新进化笔记中建议的版本
    note = load_latest_evolution_note(db_path)
    if note and note.get("prompt_version_suggested"):
        _active_prompt_version = note["prompt_version_suggested"]
        return _active_prompt_version

    _active_prompt_version = f"{PROMPT_BASE_VERSION}.v1"
    return _active_prompt_version


def _bump_prompt_version(current: str) -> str:
    """将版本号末尾的 v{n} 递增为 v{n+1}。"""
...
def _set_prompt_version(version: str) -> None:
    global _active_prompt_version
...
def extract_market_patterns(
    ...
    """
    from stock_recap.llm.backends import call_llm
    ...
    messages = [
        {
            "role": "system",
            "content": "你是A股市场分析专家。请从以下历史复盘结论中提炼持续出现的市场规律（持续强势板块、持续的资金方向、近期主要矛盾），用3-5条要点输出，每条不超过50字。",
        },
    ...
    try:
        from stock_recap.llm.backends import _stable_json as sj
        import httpx
...
def _run_evolution(
    ...
    if new_version:
        _set_prompt_version(new_version)
        logger.info(
            _stable_json(
                {
